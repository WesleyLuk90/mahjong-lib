...
class Tile {
    ...
    static all() {
        ...
        SUIT_ORDER.forEach((suit) => {
            for (let i = 1; i <= TILES_PER_SUIT; i++) {
                if (suit !== Tile.HONORS || i <= 7) {
                    tiles.push(new Tile(i, suit));
                }
            }
        });
        return tiles;
    ...
    static fromHashCode(index) {
        assert(index >= 0 && index <= Tile.MAX_TILE_HASH, `Invalid hash code ${index}`);
    ...
    validate() {
        if (this.suit === Tile.HONORS) {
            assert(1 <= this.number && this.number <= 7, `Got invalid honor tile number ${this.number}`);
            ...
            assert.fail(`Invalid suit ${this.suit}`);
        }
    }
